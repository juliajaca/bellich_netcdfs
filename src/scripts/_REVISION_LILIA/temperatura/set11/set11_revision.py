...
# %% COMPROBACION
dataset = Dataset(f'C:/Users/Julia/Documents/VSCODE_BELLICH/src/scripts/_REVISION_LILIA/temperatura/set11/set11_IEO/set11_TEMP_IEO.nc', "r")
...
# %%
idx_tiempo = np.where(fechas == np.datetime64('2016-09-19'))[0]
...
plt.title('Temperatura a -0 m el 19 de sept de 2016')
plt.xlabel('Longitud')
plt.ylabel('Latitud')
plt.grid(True)
plt.show()
...
idx_lat = np.where(vars_nc['latitude'].data == [37.792949])
idx_lon = np.where(vars_nc['longitude'].data == [-0.7833])
...
# %% SERIE TEMPORAL
idx_depth = np.where(vars_nc['depth'].data == 2)[0]
