...
# %%
dict = {
    'clorofila (espectofotometria)':{'v_name': 'chlorophyll',
    ...
    'nitrato':{'v_name': 'nitrate',
                'long_name':'mass_concentration_of_nitrate_in_sea_water'},
    'nitrito':{'v_name': 'nitrite',
            'long_name': 'mass_concentration_of_nitrite_in_sea_water'},
    'fosfato':{'v_name': 'phosphate',
            'long_name': 'mass_concentration_of_phosphate_in_sea_water'},
    'silicato':{'v_name': 'silicate',
                'long_name': 'mass_concentration_of_silicate_in_sea_water'},
    'amonio':{'v_name': 'ammonium',
            'long_name': 'mass_concentration_of_ammonium_in_sea_water'},
    'NID':{'v_name': 'nid',
            'long_name': 'dissolved inorganic nitrogen'},
    'relacion NP':{'v_name': 'NPrelation',
                'long_name': 'N:P molar ratio',
                'comment':'Molar ratio of dissolved inorganic nitrogen to phosphate ',
                 'units': 1 },
}
...
path = 'C:/Users/Julia/Nextcloud/Datos_MM_Art_2025/datasets_ncFormat/Biogeochemical/set9/'
...
# %%
def crear_nc(data, dias_desde_1970, nombre_variable):
    # nombre_variable_nc = nombre_variable.replace(":", "_")
    print(nombre_variable)
    ncfile = Dataset(f'{path}resumen_belich_bg_{nombre_variable}.nc', mode='w', format='NETCDF3_CLASSIC')

    ncfile.title=f'resumen_belich_bg_{nombre_variable}'
    ...
    value_var = ncfile.createVariable(dict[nombre_variable]['v_name'], np.float64, ('time', 'station_name'))
    value_var.long_name = dict[nombre_variable]['long_name']
    
    com = dict[nombre_variable].get('comment')
    if com:
        value_var.comment = com
    value_var.units = dict[nombre_variable].get('units', '¿?¿')
...
# %% COMPROBACION
def comprobar_nc(nombre_variable):
    # nombre_variable_nc = nombre_variable.replace(":", "_")
    dataset = Dataset(f'{path}resumen_belich_bg_{nombre_variable}.nc', "r")
...
# nombre_variable= 'nitrato'
    # if nombre_variable == 'relacion N:P':
        ...
        generar_txt(f'{path}resumen_belich_bg_{nombre_variable}.nc', f'{path}resumen_belich_bg_{nombre_variable}_display2.txt')
