...
nombre_fichero = 'IEO_COMU_2_OXY_SAT' #TESIS DE Juli
...
# %%
oxy = data0.loc[(data0['var'] == 'O2 sat') & (data0['Referencia'] == 'Excel JM86')]
print(data0.head())
...
value_var = ncfile.createVariable('oxygen_saturation', np.float32, ('time', 'station_name','level'))
value_var.units = '%'
...
for i_station, station in enumerate(stations):
    ...
    plt.xlabel('Fecha')
    plt.ylabel('O2 SAT')
    plt.title(f'Serie temporal de oxygen_saturation en {station} en {northing[i_station]} y {easting[i_station]}')
    ...
    file_name = f'{station}_temporal_profile_oxy sat.png'
