...
# %%
data = pd.read_csv('C:/Users/Julia/Documents/VSCODE_BELLICH/src/scripts/descarga_rios/actualizacion2026/datos/CHS/CHS_Albujon_flow_daily.csv', dtype={  
"Flow_mean": "float64", "Discharge":"float64" }, parse_dates= ['Date_time'])

nombre_fichero= 'CHS_Albujon_FLOW_DAILY'
# %%
...
ncfile.comment = ('''
    Continuous sensor recording. Water level measured with radar time-of-flight sensor Endress+Hauser Micropilot FMR20. Flow is the average of one measurement per second, and highest resolution data are offered as 5-minute means. Note that the value is not necessarily the flow discharging in the Mar Menor, as the gauge is located 200 m from the actual outlet into the lagoon and is not accounting the water diverted to El Albujón pump station to conduct the water to the desalination plant of El Mojón (16 km NE). In general, most of the days referred to in these data the pumping was not operating. This effect is negligible in high flows. However, during floods part of the flow of the tributary named D7 is routed to an alternative non-gauged outlet, and, therefore, typically during peaks of floods not all the discharge of the catchment is recorded in the outlet of the rambla del Albujón, were the flow gauge is located
'''
)
...
disch_var.comment =  "Daily accumulated discharge computed from daily mean flow"
...
axs[0].set_title("Daily Mean River Flow")
axs[0].grid(True)

axs[1].plot( fechas,disch,color="tab:orange",linewidth=1)
axs[1].set_ylabel("Daily Discharge (m³)")
...
plt.savefig(
    f"C:/Users/Julia/Nextcloud/Datos_MM_Art_2025/datasets_ncFormat/Runoff/CHS_Albujon/flow_discharge_timeseries.png",
